...
Codex実装中に要件変更が出た場合は、Notionの状態を `要件整理中` または `承認待ち` に戻します。変更内容、理由、影響範囲、完了条件の差分をNotionメモに記録し、ユーザーのYES承認を取り直してから実装を再開します。

既存PRがある場合は、PRコメントに要件変更の要約を残します。MVPに不要な機能追加や税務判断に見える変更は、追加実装せず差戻し扱いにします。

### ClaudeレビューNG時の差戻し

Claude CodeレビューでNGになった場合は、Notionの状態を `差戻し` または `レビュー中` にし、PRコメントとNotionメモの両方に指摘内容を残します。

Codexは指摘範囲だけを修正し、修正後に再レビューを依頼します。設計変更、セキュリティ懸念、companyIdスコープ不備、税務・会計ロジックの断定表現がある場合は、人間確認が終わるまでマージしません。

### ユーザーテストNG時のロールバック運用

ユーザーテストでNGになった場合は、Notionの状態を `差戻し` または `テスト待ち` にし、再現手順、期待結果、実際の結果、停止条件を記録します。

未マージPRの場合は、修正コミットを追加して再テストします。マージ後に重大な問題が判明した場合は、直前の安定版へ戻すPRを作成するか、影響機能を停止する運用を優先します。secret/token/API key や個人情報が露出した疑いがある場合は、公開停止と権限ローテーションを人間判断にエスカレーションします。
